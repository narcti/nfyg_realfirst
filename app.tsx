...
import { generateDecadeImage, generateBaseImageFromPrompt } from './services/geminiService';
...
function App() {
    ...
    const [appState, setAppState] = useState<'idle' | 'generating-base-image' | 'image-uploaded' | 'generating' | 'results-shown'>('idle');
    ...
    const handleSearch = async (e: React.FormEvent) => {
        ...
        setAppState('generating-base-image');
        setSearchError(null);
        setUploadedImage(null);
        setGeneratedImages({});

        try {
            const resultUrl = await generateBaseImageFromPrompt(searchQuery);
            setUploadedImage(resultUrl);
            setAppState('image-uploaded');
        } catch (err) {
            console.error(err);
            const errorMessage = err instanceof Error ? err.message : "Failed to create an image. Please try a different search term.";
    ...
    const handleDownloadIndividualImage = (decade: string) => {
        // Fix: Explicitly type `image` to resolve incorrect `unknown` type inference.
        const image: GeneratedImage | undefined = generatedImages[decade];
    ...
    return (
        <main className="bg-black text-neutral-200 min-h-screen w-full flex flex-col items-center justify-center p-4 pb-24 overflow-hidden relative">
            ...
            <div className="z-10 flex flex-col items-center justify-center w-full h-full flex-1 min-h-0">
                        ...
                        >
                            <form onSubmit={handleSearch} className="w-full flex flex-col items-center gap-4">
                               <input 
                                   ...
                                   placeholder="e.g., a smiling astronaut, a 1920s detective..."
                                   className="font-permanent-marker text-xl text-center placeholder:text-neutral-600 bg-white/5 backdrop-blur-sm border-2 border-white/80 rounded-sm w-full py-4 px-6 text-white focus:ring-2 focus:ring-yellow-400 focus:outline-none transition-all"
                                   aria-label="Image search query"
                               />
                               <button type="submit" className={primaryButtonClasses} disabled={!searchQuery.trim()}>
                                   Create Photo
                               </button>
                            </form>
                            <p className="mt-8 font-permanent-marker text-neutral-500 text-center max-w-md text-lg">
                                Describe a person, and we'll create their photo to start your journey through time.
                            </p>
                             {searchError && (
                                <p className="mt-4 text-red-400 bg-red-900/50 border border-red-400/50 p-3 rounded-md text-center max-w-md">
                ...
                {appState === 'generating-base-image' && (
                    <div className="flex flex-col items-center gap-4">
                        <PolaroidCard 
                           caption="Creating your photo..."
                           status="pending"
                        />
                        <p className="font-permanent-marker text-neutral-400 text-lg animate-pulse">
