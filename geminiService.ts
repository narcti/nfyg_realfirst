...
/**
 * Generates a base image from a text prompt using the Imagen model.
 * @param prompt The user's search query.
 * @returns A promise that resolves to a base64-encoded image data URL.
 */
export async function generateBaseImageFromPrompt(prompt: string): Promise<string> {
  console.log(`Generating base image for prompt: "${prompt}"`);
  try {
    const response = await ai.models.generateImages({
      model: 'imagen-4.0-generate-001',
      prompt: `A photorealistic, high-quality, front-facing portrait of ${prompt}. The person should be clearly visible against a neutral background.`,
      config: {
        numberOfImages: 1,
        outputMimeType: 'image/jpeg',
        aspectRatio: '1:1', // Polaroid-friendly aspect ratio
      },
    });
    ...
    if (response.generatedImages && response.generatedImages.length > 0) {
      ...
      throw new Error("The AI model did not return any images. Please try a different search term.");
    }
  } catch (error) {
